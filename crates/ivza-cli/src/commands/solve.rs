...
pub async fn run(args: SolveArgs, _cfg: &CliConfig) -> Result<()> {
    ...
    let solution = match args.strategy.as_str() {
        ...
        other => bail!("Unknown strategy '{}'. Use 'balanced' or 'critical-path'.", other),
    };
        ...
        for i in 0..candidate.len() {
            ...
            for l in 0..lane_count {
                ...
                let dep_ready = rev_adj[candidate[i].node_index as usize]
                    .iter()
                    .filter_map(|&pred| {
                        candidate.iter().find(|s| s.node_index == pred).map(|s| s.end_time)
                    })
                    .max()
                    .unwrap_or(0);
        ...
        for &succ in &adj[ni] {
            ...
            if in_deg[succ as usize] == 0 {
                ...
                let pos = ready
                    .iter()
                    .position(|&r| {
                        priority_rank.get(&r).copied().unwrap_or(usize::MAX) > rank
                    })
                    .unwrap_or(ready.len());
...
fn build_graph_raw(
    intent: &IntentInput,
    n: usize,
) -> (Vec<Vec<u16>>, Vec<Vec<u16>>, Vec<u16>) {
    let mut adj = vec![Vec::new(); n];
    let mut rev_adj = vec![Vec::new(); n];
    let mut in_degree = vec![0u16; n];
...
fn print_solution(sol: &SolverSolution) {
    ...
    for lt in &sol.lane_timelines {
        ...
        for slot in &lt.slots {
            let width = ((slot.end - slot.start) as f64 / sol.makespan.max(1) as f64 * 40.0) as usize;
