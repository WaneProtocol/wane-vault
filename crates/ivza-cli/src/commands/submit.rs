...
pub async fn run(args: SubmitArgs, cfg: &CliConfig) -> Result<()> {
    ...
    let program_id = Pubkey::from_str(&cfg.program_id)
        .context("Invalid program ID in configuration")?;
    ...
    let (graph_pda, _) = Pubkey::find_program_address(
        &[SEED_GRAPH, graph_id.as_ref()],
        &program_id,
    );
...
fn print_graph_summary(intent: &IntentInput, graph_id: &[u8; 32], lane_count: u8) {
    ...
    for (i, node) in intent.nodes.iter().enumerate() {
        println!(
            "  [{}] {} (est. {} CU)",
            i, node.label, node.estimated_cu
        );
    }
    println!();
    println!("Edges:");
    for edge in &intent.edges {
        println!(
            "  {} -> {}  ({} -> {})",
            edge[0], edge[1], intent.nodes[edge[0] as usize].label, intent.nodes[edge[1] as usize].label,
        );
    }
    println!("========================================");
}
