...
use crate::types::{AccountAccessTracker, AccountConflict, AccountSet, DependencyType, NodeId};
...
impl DependencyAnalyzer {
    ...
    pub fn analyze(&self, graph: &TransactionGraph) -> Result<TransactionGraph> {
        info!(
            "Analyzing dependencies for {} nodes",
            graph.node_count()
        );
        ...
        let existing_edges: HashSet<(NodeId, NodeId)> = graph
            .edges
            .iter()
            .map(|e| (e.from, e.to))
            .collect();
