...
use analyzer::{CriticalPathAnalyzer, CriticalPathResult, DependencyAnalyzer, ParallelismAnalyzer, ParallelismResult};
...
impl IvzaEngine {
    ...
    pub fn decompose(&self, instructions: Vec<InstructionData>) -> Result<TransactionGraph> {
        info!("IvzaEngine: decomposing {} instructions", instructions.len());
    ...
    pub fn parallelism_summary(
        ...
        let (seq_cost, par_cost, ratio) =
            self.parallelism_analyzer.parallelism_ratio(&analyzed.graph)?;
...
impl std::fmt::Display for ParallelismSummary {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        writeln!(f, "Nodes: {}, Edges: {}", self.total_nodes, self.total_edges)?;
        writeln!(
            f,
...
mod tests {
    ...
    fn test_graph_builder_and_topo_sort() {
        ...
        builder.add_edge(n1, n2, DependencyType::DataDependency).unwrap();
    ...
    fn test_execution_planner() {
        ...
        let ix2 = make_ix(program, &[make_pubkey(1), make_pubkey(2)], &[make_pubkey(3)], "rw");
