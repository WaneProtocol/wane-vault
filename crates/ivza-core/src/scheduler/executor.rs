...
use crate::analyzer::{CriticalPathAnalyzer, ParallelismAnalyzer};
...
impl ExecutionPlanner {
    ...
    pub fn plan_optimized(&self, graph: &TransactionGraph) -> Result<ExecutionPlan> {
        ...
        info!("Optimizing execution plan with {} lanes", base_plan.lanes.len());
        ...
        for i in 1..base_plan.lanes.len() {
                ...
                && current_lane.total_cu + next.total_cu <= self.max_lane_cu
            {
                ...
                debug!("Merged lane {} into lane {}", next.index, current_lane.index);
            } else {
