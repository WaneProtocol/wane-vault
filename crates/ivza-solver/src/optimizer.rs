...
use crate::pool::{calculate_optimal_split, calculate_output, calculate_price_impact, PoolRegistry};
use crate::router::{Route, RouteEngine, RouteHop};
...
impl ExecutionOptimizer {
    ...
    pub fn optimize(
        ...
        let optimized_output: u64 = optimized_swaps
            ...
            .map(|s| s.route.output_amount)
            .sum();
        let optimized_input: u64 = optimized_swaps
            .values()
        ...
        let total_hops: usize = optimized_swaps
            .values()
            .map(|s| s.route.hop_count())
            .sum();
        let estimated_cost =
            5_000u64 * optimized_swaps.len() as u64 + 200 * total_hops as u64;
    ...
    fn combine_split_routes(
        ...
        let best_split = splits
            .iter()
            .max_by_key(|s| s.route.output_amount)
            .unwrap();
